...
## Quick local check: do Stage 04 outputs exist?

```{r}
root_04 <- file.path("data", "processed", "04_grid")
if (!dir.exists(root_04)) {
  tibble::tibble(stage04_found = FALSE, note = "No data/processed/04_grid folder found yet.")
} else {
  # Show the first few grid-spec folders if present
  specs <- list.dirs(root_04, full.names = FALSE, recursive = FALSE)
  tibble::tibble(
    stage04_found = TRUE,
    n_grid_specs = length(specs),
    grid_specs = paste(head(specs, 8), collapse = ", ")
  )
}
```
