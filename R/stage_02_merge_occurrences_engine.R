# Filename: stage_02_merge_occurrences_engine.R
#
# InfluentialSpecies - Stage 02: merge and safe 1-to-1 cross-source auto-drop of duplicates -----
#
# Outputs:
...
#
# Behaviour:
#   - Reads raw-clean CSVs created by pull_raw_occurrences() (+ Stage 1.6 NBN top-up/repair clean rewrites)
...
#   - We always set a canonical `source` ("GBIF"/"NBN") based on the file being read.
#     If the input already had a `source` column, we preserve it as `source_raw`.
#
# Performance:
...
# ---- Helper: best-effort script path -----------------------------------------
# Used only to make repo-root discovery resilient when running from long-run wrappers
# or via Rscript (where getwd() is not guaranteed to be the repo root).
get_script_path <- function() {
  ...
  # source("...") sets sys.frames()[[i]]$ofile in many contexts
  ...
  # RStudio active document (optional)
...
# ---- Helper: repo root -------------------------------------------------------
# We consider "repo root" to be a folder that contains /data and /scripts.
get_repo_root <- function() {
  walk_up <- function(start_dir) {
    d <- normalizePath(start_dir, winslash = "/", mustWork = FALSE)
    if (is.na(d) || !nzchar(d)) return(NA_character_)
    repeat {
      if (dir.exists(file.path(d, "data")) && dir.exists(file.path(d, "scripts"))) return(d)
      parent <- dirname(d)
  ...
  # Try working directory first (still convenient for interactive use)
  hit <- walk_up(getwd())
  if (!is.na(hit)) return(hit)
  
  # Try directory of this script (robust for long-run usage)
  sp <- get_script_path()
  if (!is.na(sp) && nzchar(sp)) {
    hit2 <- walk_up(dirname(sp))
    if (!is.na(hit2)) return(hit2)
  }
  
  stop(
    "Can't locate repo root.\n",
    "Looked upward from getwd() and (if available) this script's directory.\n",
    "Expected to find both /data and /scripts.\n",
    "If running interactively, set working directory to the project root (InfluentialSpecies) and try again."
  )
}
...
merge_occurrences <- function(species_names,
                              ...
                              out_root = file.path("data", "processed", "02_merged"),
  ...
  prefer_source <- toupper(match.arg(prefer_source, c("GBIF", "NBN")))
  ...
  runlog_path <- file.path(out_group_dir, "_runlog_02_merged.csv")
  on.exit({
  ...
  add_basics <- function(df, source_name, slug, species_input, coord_round_dp) {
    ...
    # Canonical source: always set from the file we are reading.
    # If the input already had a `source` column, preserve it as `source_raw`.
    if ("source" %in% names(df) && !"source_raw" %in% names(df)) {
      df$source_raw <- as.character(df$source)
    }
    ...
    if (!overwrite && !is.na(out_existing)) {
      if (isTRUE(refresh_if_inputs_newer)) {
        ...
        if (!inputs_newer) {
          ...
          message("[02_merged] ", sp, " -> skipped_cached")
          next
        }
      } else {
        run_log <- add_row(
